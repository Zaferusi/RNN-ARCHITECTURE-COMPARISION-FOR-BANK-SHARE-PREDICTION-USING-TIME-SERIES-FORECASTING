...
def series_to_supervised(data, train_size=0.5, n_in=1, n_out=1, target_column='Now', dropnan=True, scale_X=True):

    df = data.copy()

    # Make sure the target column is the last column in the dataframe
    df['Moment'] = df[target_column] # Make a copy of the target column
    ...
    #Burada da Hedef veriyi X ve Y olarak ayırır. Son  sütun sayısını target yani bağımlı değişkne atar.
...
#Yapılan bu işlemler sonucunda X verilerinin normalize edilerek 0.5 lik parçasının eğitim set verisine aktarıldığı kala parçasının test veri setine ait olduğu görüldü. 
#Bizim kullandığımız daha önceki algoritmalarda zaten bu parçalanmayı validation split kullanarak gerçekleştirmiştik. O yüzden bu mod için buna ihtiyaç olduğunu düşünmüyorum.
